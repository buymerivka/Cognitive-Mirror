...
def manipulations_classify(text: str, paragraph_index: int, sentence_index: int, char_start: int, char_end: int,
                        n: int = 1):
    if n > 9:
        n = 9

    device = torch.device('mps')
    local_tokenizer_path = f'{BASE_DIR}/models/manipulations_bert_model/tokenizer'
    local_model_path = f'{BASE_DIR}/models/manipulations_bert_model/checkpoint-504'
...
def text_classify_by_sentence(text: str, n: int = 1):
    ...
    for sentence_data in sentences_data:
        result.append(
            manipulations_classify(sentence_data.text, sentence_data.paragraphIndex, sentence_data.sentenceIndex,
